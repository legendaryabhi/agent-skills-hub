...
This clones to `~/.agent/skills` by default. Use `--cursor`, `--claude`, `--gemini`, or `--codex` to install for a specific tool, or `--path <dir>` for a custom location. Run `npx agent-skills-hub --help` for details.

If you see a 404 error, use: `npx github:sickn33/agent-skills-hub`
